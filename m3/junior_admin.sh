# Set AWS profile to use anna-prod
export AWS_PROFILE=anna-prod

# We're going to manually create two new subnets

# First let's get the existing VPC id
vpc_id=$(aws ec2 describe-vpcs --filters Name="tag:Name",Values="anna1" \
...
pub_rt_id=$(aws ec2 describe-route-tables --filters Name="vpc-id",Values=$vpc_id \
  Name="tag:Name",Values="anna1-public" \
